...
@inline function unsafe_cudss_batch(batch::Vector{<:CuArray{T}}) where T <: BlasFloat
    cuPtrs = Base.unsafe_convert.(Ptr{Cvoid}, batch) |> CuVector
    return cuPtrs
end
...
@inline function unsafe_cudss_batch(batch::Vector{CuSparseMatrixCSR{T,Cint}}) where T <: BlasFloat
    rowsPtrs = [Base.unsafe_convert(Ptr{Cvoid}, Aᵢ.rowPtr) for Aᵢ in batch] |> CuVector
    colVals = [Base.unsafe_convert(Ptr{Cvoid}, Aᵢ.colVal) for Aᵢ in batch] |> CuVector
    nzVals = [Base.unsafe_convert(Ptr{Cvoid}, Aᵢ.nzVal) for Aᵢ in batch] |> CuVector
