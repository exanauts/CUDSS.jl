function cudss_batched_dense()
  ...
  @testset "cudss_update -- precision = $T" for T in (Float32, Float64, ComplexF32, ComplexF64)
    @testset "CuVector" begin
      A_cpu = rand(T, n)
      A_gpu = [CuVector(A_cpu), CuVector(A_cpu)]
      ...
      A_cpu2 = rand(T, n)
      A_gpu2 = [CuVector(A_cpu2), CuVector(A_cpu2)]
      cudss_update(matrix, A_gpu2)
    end

    @testset "CuMatrix" begin
      A_cpu = rand(T, n, p)
      A_gpu = [CuMatrix(A_cpu), CuMatrix(A_cpu)]
      matrix = CudssBatchedMatrix(A_gpu)
      format = Ref{Cint}()
      CUDSS.cudssMatrixGetFormat(matrix, format)
      @test format[] == 5  # BATCH + DENSE
      ...
      A_cpu2 = rand(T, n, p)
      A_gpu2 = [CuMatrix(A_cpu2), CuMatrix(A_cpu2)]
      cudss_update(matrix, A_gpu2)
    end
  end
end
    ...
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      ...
      @testset "view = $view" for view in ('L', 'U', 'F')
        @testset "structure = $structure" for structure in ("G", "S", "H", "SPD", "HPD")
          ...
          A_gpu2 = [CuSparseMatrixCSR{T,INT}(A_cpu2), CuSparseMatrixCSR{T,INT}(A_cpu2)]
          cudss_update(matrix, A_gpu2)
        end
      end
    end
    ...
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      ...
      @testset "Unsymmetric -- Non-Hermitian" begin
        @testset "Pivoting = $pivot" for pivot in ('C', 'R', 'N')
          ...
          @test A_gpu isa Vector{CuSparseMatrixCSR{T,INT}}
...
function batched_hybrid_memory_mode()
  function hybrid_batched_lu(T, INT, A_cpu, x_cpu, b_cpu)
    A_gpu = CuSparseMatrixCSR{T,INT}.(A_cpu)
    x_gpu = CuVector.(x_cpu)
    b_gpu = CuVector.(b_cpu)
  ...
  function hybrid_batched_ldlt(T, INT, A_cpu, x_cpu, b_cpu, uplo)
    if uplo == 'L'
  ...
  function hybrid_batched_llt(T, INT, A_cpu, x_cpu, b_cpu, uplo)
    if uplo == 'L'
      A_gpu = CuSparseMatrixCSR{T,INT}.(A_cpu .|> tril)
    elseif uplo == 'U'
      A_gpu = CuSparseMatrixCSR{T,INT}.(A_cpu .|> triu)
  ...
  @testset "precision = $T" for T in (Float32, Float64, ComplexF32, ComplexF64)
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      ...
      @testset "LU" begin
        ...
        res = hybrid_batched_lu(T, INT, A_cpu, x_cpu, b_cpu)
      ...
      @testset "LLᵀ / LLᴴ" begin
        ...
        b_cpu = [rand(T, n[i]) for i = 1:5]
        @testset "uplo = $uplo" for uplo in ('L', 'U', 'F')
          res = hybrid_batched_llt(T, INT, A_cpu, x_cpu, b_cpu, uplo)
          @test mapreduce(r -> r ≤ √eps(R), &, res)
        end
      end
    end
...
function refactorization_batched_cholesky()
  @testset "precision = $T" for T in (Float32, Float64, ComplexF32, ComplexF64)
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      ...
      batch_A_gpu = CuSparseMatrixCSR{T,INT}[]
      ...
      for i = 1:nbatch
        ...
        push!(batch_A_gpu, A_cpu |> triu |> CuSparseMatrixCSR{T,INT})
      ...
      @assert batch_A_gpu isa Vector{CuSparseMatrixCSR{T,INT}}
