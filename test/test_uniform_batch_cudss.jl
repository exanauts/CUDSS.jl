function cudss_uniform_batch_lu()
  @testset "precision = $T" for T in (Float32, Float64, ComplexF32, ComplexF64)
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      ...
      cudss("factorization", solver, cudss_xλ_gpu, cudss_bλ_gpu; asynchronous=true)
      cudss("solve", solver, cudss_xλ_gpu, cudss_bλ_gpu; asynchronous=true)
      ...
      cudss("refactorization", solver, cudss_xλ_gpu, cudss_bλ_gpu; asynchronous=true)
      cudss("solve", solver, cudss_xλ_gpu, cudss_bλ_gpu; asynchronous=true)
  ...
  @testset "precision = $T" for T in (Float32, Float64, ComplexF32, ComplexF64)
    @testset "integer = $INT" for INT in (Cint,) # Int64)
      @testset "Triangle of the sparse matrix = $uplo" for uplo in ('L', 'U', 'F')
        ...
        cudss("factorization", solver, cudss_Xs_gpu, cudss_Bs_gpu; asynchronous=true)
        cudss("solve", solver, cudss_Xs_gpu, cudss_Bs_gpu; asynchronous=true)
        ...
        cudss_update(solver, rowPtr, colVal, new_nzVal)
        cudss("refactorization", solver, cudss_Xs_gpu, cudss_Bs_gpu; asynchronous=true)
